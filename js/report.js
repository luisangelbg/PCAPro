...
Rep.build = opts => {
  ...
  if (inc('metodos')) {
    ...
    body += `<section><h2>${tt('Métodos')}</h2><p>` + TT(`
      ...
      Todos los cálculos se realizaron en PCAPro, que implementa la descomposición espectral por
      rotaciones de Jacobi y las rotaciones factoriales por el algoritmo de proyección de gradiente.
    `, `
      ...
      All computations were carried out in PCAPro, which implements the spectral decomposition by
      Jacobi rotations and the factor rotations by the gradient projection algorithm.
    `) + `</p></section>`;
  }
